...
function init() {
  filters = {
    "fa-solid fa-sun": { value: 100, max: 200 },
    "fa-sharp fa-solid fa-circle-half-stroke": { value: 100, max: 200 },
    "fa-solid fa-droplet": { value: 100, max: 200 },
    "fa-solid fa-swatchbook": { value: 0, max: 100 },
    "fa-solid fa-fill-drip": { value: 0, max: 100 },
  };
...
ButtonsFilter.forEach((item) => {
  item.onclick = () => {
    ...
    filterActive = iconElement.getAttribute("class");

    range.max = filters[filterActive]?.max || 100;
    range.value = filters[filterActive]?.value || 0;
...
function applyFilters() {
  img.style.filter = `
    brightness(${filters["fa-solid fa-sun"].value}%) 
    contrast(${filters["fa-sharp fa-solid fa-circle-half-stroke"].value}%) 
    saturate(${filters["fa-solid fa-droplet"].value}%) 
    grayscale(${filters["fa-solid fa-swatchbook"].value}%) 
    invert(${filters["fa-solid fa-fill-drip"].value}%)
  `;
}